...
func (add *Addon) HandleGetManifest(c *fiber.Ctx) error {
	...
	manifest := &Manifest{
		...
		IDPrefixes: []string{"tt"},
		Logo:       appendAuthToken(c.BaseURL()+"/logo", add.authToken),
...
func (add *Addon) HandleGetStreams(c *fiber.Ctx) error {
	...
	for _, r := range records {
		...
		if r.RDClient != nil && len(r.Files) > 0 {
			...
			streamURL := appendAuthToken(r.BaseURL+compiled.ReplaceAllString(c.Path(), "/download/"+r.Torrent.InfoHash+"/"+r.MediaFile.ID), add.authToken)
